...
export default function JobList({
  ...
  const deleteJob = async (jobId: string) => {
    if (!window.confirm("Are you sure you want to completely delete this job and all its tasks/logs? This action cannot be undone.")) return;
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403) alert("You are not authorized to delete this job.");
        else throw new Error("Failed to delete job.");
      }
      await fetchJobs();
    } catch (err) {
      console.error(err);
                      ...
                      <div className="flex gap-2 items-center lg:opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        {isAdmin && (
                          <button
                            ...
                            className="flex items-center justify-center p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 hover:border-red-200 rounded-lg bg-white border border-gray-200 shadow-sm transition-all disabled:opacity-50"
                            title="Delete Job"
                          >
                            {deletingId === job.id ? (
